...
def register_book():
    ...
    combo.set("SELECT_FROM_DROPDOWN")
...
def insert_to_books():
    ...
    try:
        if Id:
            query = "SELECT category, title, author, publisher, year_of_publish, isbn, pages, translated_by, book_genre, description FROM books WHERE book_id=?"
            ...
            if fetched is None:
                ...
                combo.set(str(i[0]))
                title_entry.delete(0, tk.END)
                title_entry.insert(tk.END, str(i[1]))
                author_entry.delete(0, tk.END)
                author_entry.insert(tk.END, str(i[2]))
                publisher_entry.delete(0, tk.END)
                publisher_entry.insert(tk.END, str(i[3]))
                year_of_publish_entry.delete(0, tk.END)
                year_of_publish_entry.insert(tk.END, str(i[4]))
                isbn_entry.delete(0, tk.END)
                isbn_entry.insert(tk.END, str(i[5]))
                pages_entry.delete(0, tk.END)
                pages_entry.insert(tk.END, str(i[6]))
                translated_by_entry.delete(0, tk.END)
                translated_by_entry.insert(tk.END, str(i[7]))
                book_genre_entry.delete(0, tk.END)
                book_genre_entry.insert(tk.END, str(i[8]))
                description_entry.delete(0, tk.END)
                description_entry.insert(tk.END, str(i[9]))
        else:
            messagebox.showerror("error", "Seems local database is raw and fresh. Therefore, no sorta detail were currently returned!")
            return
    except UnboundLocalError:
...
def user_check():
    username = username_entry.get()
    if username == "":
        messagebox.showerror("Empty!", "Please fill at least the username field!\n\nHint: This feature is intended to show complete information of a specified user all in a messagebox and in order to achieve this, you must specify the user by their username in the left pane section of the window!")
        return
    conn = sqlite3.connect("registry.db")
    cursor = conn.cursor()
    cursor.execute("SELECT username FROM users")
    all_from_users = cursor.fetchall()
    user = []
    for sub1 in all_from_users:
        for sub2 in sub1:
            user.append(sub2)
    if username in user:
        pass
    else:
        messagebox.showerror("Doesn't exist", "This User doesn't exist in the database!")
        return
    cursor.execute("SELECT * FROM users WHERE username=?", (username,))
    all_from_user = cursor.fetchall()
    for items in all_from_user:
        items = items
    messagebox.showinfo(f"User {username}'s info --------->", f"Username:   {items[0]}\n\n\nName:   {items[1]}\n\n\nMiddle Name & Last Name:   {items[2]}\n\n\nAge:   {items[3]}\n\n\nAddress:   {items[4]}\n\n\nPhone Number:   {items[5]}\n\n\nNational ID:   {items[6]}\n\n\nEmail:   {items[7]}\n\n\nPostal Code:   {items[8]}")

def treeview():
    ask = messagebox.askyesno("confirmation", "You are opening the library registry environment in a treeview and the read-only mode!\n\n__PROCEED?__")
    if ask:
        subprocess.Popen(['python', 'Library_Registry/Treeview.py'])
    else:
        return
...
def book_check():
    book_id = book_id_entry.get()
    if book_id == "":
        messagebox.showerror("Empty!", "Please fill at least the Book ID field!\n\nHint: This feature is intended to show complete information of a specified Book all in a messagebox and in order to achieve this, you must specify the Book by their ID in the right pane section of the window!")
        return
    conn = sqlite3.connect("registry.db")
    cursor = conn.cursor()
    cursor.execute("SELECT book_id FROM books")
    all_from_books = cursor.fetchall()
    for book in all_from_books:
        book = str(book)
    if book_id in book:
        pass
    else:
        messagebox.showerror("Doesn't exist", "This Book doesn't exist in the database!")
        return
    cursor.execute("SELECT * FROM books WHERE book_id=?", (book_id,))
    all_from_book = cursor.fetchall()
    for items in all_from_book:
        items = items
    messagebox.showinfo(f"Book Title : <{items[2]}>'s info ------>", f"Category:   {items[0]}\n\n\nBook ID:   {items[1]}\n\n\nTitle:   {items[2]}\n\n\nAuther:   {items[3]}\n\n\nPublisher:   {items[4]}\n\n\nYear of Publish:   {items[5]}\n\n\nISBN:   {items[6]}\n\n\nPages:   {items[7]}\n\n\nTranslator:   {items[8]}\n\n\nBook Genre:   {items[9]}\n\n\nDescription:   {items[10]}")
...
middleframe3.place(relx=0.5, rely=0.76, anchor=tk.N)
...
combo = ttk.Combobox(right_pane, width=20, justify="center", height=20)
style2 = ttk.Style(right_pane)
style2.configure('TMenubutton', foreground='blue', background='lightgreen')
combo['values'] = ("Engineering", "Meta Physics", "Mathematics", "Motivational", "Narrative", "Comedy", "Memoir", "Fiction", "History", "Poetry", "Science", "Novel", "Phsychology", "Romance","Academic", "Entertainment", "Science Fiction", "IT and Technology", "Biology", "Religious", "Spirituality", "Artificial Intelligence", "Correlated Artbook", "Geography", "Astrology", "Crime Fiction", "Antropology", "Robotics", "Graphics", "Video Game Franchise Books", "Relationship-Help", "Children", "Folklore", "Music", "Survival-Horror Fiction", "Self Improvement", "Historical Fiction", "Biography", "Thriller", "Inspired by True stories", "Mystery", "Art", "Ethnic and Culture", "Fantasy", "Other")
combo.set("SELECT_FROM_DROPDOWN")
combo.configure(style='TMenubutton', font="arial 9 bold")
...
books_insert_button.place(relx=0.89, rely=0.9671, anchor=tk.N)

upper_frame = tk.Frame(window)
upper_frame.config(bg="darkblue")
upper_frame.pack(side=tk.TOP, pady=6)

additional_frame = tk.Frame(window, bg="darkblue")
user_view_button = tk.Button(additional_frame, bg="#00ff00", text="User Check", height=2, fg="blue", font="arial 12 bold", relief="groove", command=user_check)
user_view_button.pack(side="left", padx=3)
treeview_button = tk.Button(additional_frame, bg="#00ff00", text="Tree View", height=2, fg="blue", font="arial 12 bold", relief="groove", command=treeview)
treeview_button.pack(side="left", padx=3)
book_view_button = tk.Button(additional_frame, bg="#00ff00", text="Book Check", height=2, fg="blue", font="arial 12 bold", relief="groove", command=book_check)
book_view_button.pack(side="left", padx=3)
additional_frame.place(relx=0.5, rely=0.86, anchor=tk.N)
